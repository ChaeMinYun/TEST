...
def chat():
    ...
    if image:
        try:
            ...
            # 예를 들어, 이미지의 기본적인 특성을 설명하는 문자열을 생성합니다.
            image_description = f"이미지가 업로드되었습니다. 크기: {img.size}, 형식: {img.format}"
        except Exception as e:
            image_description = f"이미지 처리 중 오류 발생: {str(e)}"


        combined_input = f"{user_message}\n{image_description}".strip()
        
    # 네이버 쇼핑 API로 상품 정보 가져오기
    items = get_naver_shopping_data(combined_input)
    ...
    def generate():
        ...
        # 메모리 업데이트
        memory.save_context({"human_input": combined_input}, {"output": full_response})
